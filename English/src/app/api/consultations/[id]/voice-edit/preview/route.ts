...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!(file instanceof File)) {
      throw new AppError("INVALID_VOICE_EDIT_FILE", "Voice edit audio is required.", 400);
    }
    ...
    const appError = isAppError(error)
      ? error
      : new AppError("VOICE_EDIT_PREVIEW_FAILED", "Voice edit preview could not be created.", 500);
