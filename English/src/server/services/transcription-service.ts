...
export async function transcribeConsultationAudio(consultationId: string, audioAssetId: string) {
  ...
  if (audioError || !audioAsset) {
    throw new AppError("AUDIO_ASSET_NOT_FOUND", "Audio asset could not be found.", 404);
  }
  ...
  if (downloadError || !blobData) {
    throw new AppError("AUDIO_DOWNLOAD_FAILED", "Stored audio could not be read.", 500);
  }
  ...
  if (transcriptionRecord.error || !transcriptionRecord.data) {
    throw new AppError("TRANSCRIPT_CREATE_FAILED", "Transcript record could not be created.", 500);
  }
  ...
  try {
    ...
    throw new AppError("TRANSCRIPTION_FAILED", "Transcription could not be completed.", 500);
  }
}
...
export async function transcribeInstructionAudio(
  ...
  if (!text) {
    throw new AppError("VOICE_EDIT_TRANSCRIPTION_EMPTY", "Voice edit instruction could not be transcribed.", 400);
  }
