...
export async function initiateAudioUpload(
  ...
  if (error || !data) {
    throw new AppError("AUDIO_UPLOAD_INIT_FAILED", "Audio upload could not be initiated.", 500);
  }
...
export async function completeAudioUpload(
  ...
  if (!input.storagePath.startsWith(`${organisationId}/${consultationId}/`)) {
    throw new AppError("INVALID_STORAGE_PATH", "Audio path is not valid for this consultation.", 400);
  }
  ...
  if (error || !data) {
    throw new AppError("AUDIO_ASSET_CREATE_FAILED", "Audio metadata could not be stored.", 500);
  }
