...
export async function exportNote(input: {
  ...
  if (!note || note.status !== "approved" || !note.approved_at) {
    throw new AppError("NOTE_NOT_APPROVED", "Only approved notes can be exported.", 400);
  }
  ...
  if (uploadResult.error) {
    throw new AppError("PDF_UPLOAD_FAILED", "PDF export could not be stored.", 500);
  }
  ...
  if (error || !exportRecord) {
    throw new AppError("EXPORT_RECORD_FAILED", "Export metadata could not be stored.", 500);
  }

  const signed = await adminSupabase.storage.from(bucket).createSignedUrl(storagePath, 300);
  if (signed.error || !signed.data) {
    throw new AppError("PDF_SIGN_URL_FAILED", "PDF download URL could not be created.", 500);
  }
...
export async function listExports() {
  ...
  if (error) {
    throw new AppError("EXPORTS_FETCH_FAILED", "Exports could not be loaded.", 500);
  }
